module.exports = {
  ...
  init() {
    ...
    // Add touch events for swipping
    const wrapper = document.querySelectorAll('.js-wrapper')[0];
    wrapper.addEventListener('touchstart', this.onTouchStart.bind(this), false);
    wrapper.addEventListener('touchend', this.onTouchEnd.bind(this), false);
  ...
  onTouchEnd(event) {
    ...
    const delta = this.startTouchX - endTouchX;
    if (delta > 0) {
      this.next();
    } else {
      this.previous();
